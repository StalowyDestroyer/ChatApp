...
export const Conversation_message_component: React.FC<props> = ({
  data,
  conversationID,
}) => {
            ...
            <div className="d-flex flex-column gap-2">
              {data.message.messageFiles.map((file) =>
                file.type.startsWith("image/") ? (
                  <img
                    src={file.path}
                    className="message_image rounded"
                    key={file.id}
                  />
                ) : (
                  <div
                    key={file.id}
                    className="d-flex gap-2 other_file align-items-center"
                    onClick={() =>
                      downloadFileCilck({ id: file.id, name: file.orginalName })
                    }
                  >
                    <FontAwesomeIcon
